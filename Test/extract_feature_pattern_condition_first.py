...
def load_model(name, checkpoint_file, GeM_p):
    ...
    if name == "vit_base_pattern_condition_first":
        import models
        model = models.create(name, num_features=0, dropout=0, num_classes=4)
        model.classifier = None
        model.classifier_1 = None
    ...
    if name == "vit_base_pattern_condition_first_dgg" or name == "vit_large_pattern_condition_first_dgg" or name=="swin_base_dgg":
        import models
        model = models.create(name, num_features=0, dropout=0, num_classes=4)
        model.classifier = None
        model.classifier_1 = None
    ...
    if name == "vit_base_pattern_condition_first_dggse":
        import models
        model = models.create(name, num_features=0, dropout=0, num_classes=4)
        model.classifier = None
        model.classifier_1 = None
    ...
    if name == "vit_base_pattern_condition_first_add":
        import models
        model = models.create(name, num_features=0, dropout=0, num_classes=4)
        model.classifier = None
        model.classifier_1 = None
...
class ImageList(Dataset):
    ...
    def __init__(self, image_list, support_list, support_list_o, blur, bw, imsize=None, transform=None):
        Dataset.__init__(self)
        self.image_list = image_list
        self.support_list = support_list
        self.support_list_o = support_list_o
    ...
    def __getitem__(self, i):
        ...
        x_support = Image.open(self.support_list[i])
        x_support = x_support.convert("RGB")
        
        x_support_o = Image.open(self.support_list_o[i])
        x_support_o = x_support_o.convert("RGB")
        
        
        
        if self.blur:
            x = GaussianBlur([5, 5])(x)
            x_support = GaussianBlur([5, 5])(x_support)
            x_support_o = GaussianBlur([5, 5])(x_support_o)
        
        if self.bw:
            x = torchvision.transforms.RandomGrayscale(p=1)(x)
            x_support = torchvision.transforms.RandomGrayscale(p=1)(x_support)
            x_support_o = torchvision.transforms.RandomGrayscale(p=1)(x_support_o)
            
        if self.imsize is not None:
            x = x.resize((self.imsize,self.imsize))
            x_support = x_support.resize((self.imsize, self.imsize))
            x_support_o = x_support_o.resize((self.imsize, self.imsize))
            
        if self.transform is not None:
            x = self.transform(x)
            x_support = self.transform(x_support)
            x_support_o = self.transform(x_support_o)
        return x, x_support, x_support_o
...
def main():
    ...
    aa('--image_dir_support', default="", help="search image files in these directories")
    aa('--image_dir_support_o', default="", help="search image files in these directories")
    aa('--bind', default="", help="The support bind pkl file")
    ...
    image_dir_support = args.image_dir_support
    image_dir_support_o = args.image_dir_support_o
    ...
    print("Loading bind file!")
    with open(args.bind, 'rb') as f:
        bind = pickle.load(f)
    namess = sorted(os.listdir(image_dir))
    image_list = [image_dir + '/' + name for name in namess]
    support_list = [image_dir_support + '/' + bind[name.split('.')[0]] + '.jpg' for name in namess]
    support_list_o = [image_dir_support_o + '/' + bind[name.split('.')[0]] + '.jpg' for name in namess]
    ...
    im_dataset = ImageList(image_list, support_list, support_list_o, args.blur, args.bw, transform=transforms, imsize=args.imsize)
    ...
    with torch.no_grad():
        ...
        for i, y in enumerate(dataloader):
            x, x_support, x_support_o = y
            t0 = time.time()
            
            
            features, _ = net(x.cuda(), x_support.cuda(), x_support_o.cuda())
