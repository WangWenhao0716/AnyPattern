...
class DistributedIdentitySampler(DistributedTemplateSampler):
    ...
    def _init_data_single(self, data_source):
        ...
        target_pid = defaultdict(float)
        pid_cam = defaultdict(list)
        pid_index = defaultdict(list)

        for index, (_, pid, cam, tp) in enumerate(data_source):
            index_pid[index] = pid
            target_pid[index] = tp
        ...
        return index_pid, target_pid, pid_cam, pid_index, pids, num_samples, total_size
    ...
    def _sample_list(self, data_source, indices, index_pid, target_pid, pid_cam, pid_index, pids):
